...
# create variables for the colors of selected bar, not select bars, horizontal city average line
...
for (i in seq_along(dat$district_id)) { 
  ...
  # create the plot
  plot <- 
    ggplot(dat, aes(x=district_name, y=mean_performance, fill=selected))+
    ...
    geom_hline(yintercept = city_mean, colour = hline_color, size=1)+
    geom_text(aes(1, city_mean, label = 'average', vjust = -0.5))+
    geom_hline(yintercept = target, colour = target_color, size=1)+
    geom_text(aes(1, target, label = 'target', vjust = -0.5))+
